// ==============================================================
// File generated on Wed Dec 18 10:05:10 +0700 2024
...
#ifndef _conv2d_output_H_
#define _conv2d_output_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "conv2d_output_memcore.h"

SC_MODULE(conv2d_output) {
...
protected:
    ...
    conv2d_output_memcore* conv2d_output_memcore_U;
...
public:
    ~conv2d_output();
    SC_CTOR(conv2d_output) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        conv2d_output_memcore_U = new conv2d_output_memcore("conv2d_output_memcore_U");
        conv2d_output_memcore_U->clk      ( clk );
        conv2d_output_memcore_U->reset    ( reset );
        conv2d_output_memcore_U->ce0      ( i_ce0 );
        conv2d_output_memcore_U->we0      ( i_we0 );
        conv2d_output_memcore_U->address0 ( memcore_iaddr );
        conv2d_output_memcore_U->d0       ( i_d0 );
        conv2d_output_memcore_U->q0       ( i_q0 );

        conv2d_output_memcore_U->ce1      ( t_ce0 );
        conv2d_output_memcore_U->we1      ( t_we0 );
        conv2d_output_memcore_U->address1 ( memcore_taddr );
        conv2d_output_memcore_U->d1       ( t_d0 );
        conv2d_output_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "conv2d_output.trace");
            // ports
