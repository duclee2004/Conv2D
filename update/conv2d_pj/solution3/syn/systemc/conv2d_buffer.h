...
#ifndef _conv2d_buffer_H_
#define _conv2d_buffer_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "conv2d_buffer_memcore.h"
...
protected:
    ...
    // buffer instances
    conv2d_buffer_memcore* buffer[BufferCount];
...
public:
    ~conv2d_buffer();
    SC_CTOR(conv2d_buffer) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "conv2d_buffer_memcore_U_";
            name += szTmp;

            buffer[i] = new conv2d_buffer_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "conv2d_buffer.trace");
            // ports
